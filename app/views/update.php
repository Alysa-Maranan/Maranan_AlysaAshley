...
<body class="bg-green-100 p-8">
  <div class="max-w-lg mx-auto bg-white shadow-lg rounded-xl p-6 border border-green-300">
    <h1 class="text-3xl font-bold text-green-700 mb-6">✏️ Update Student Info</h1>
    ...
    <form action="<?=site_url('users/update/'.$user['id']);?>" method="post" class="space-y-5">
      <div>
        ...
        <input type="text" id="last_name" name="last_name" value="<?=html_escape($user['last_name']);?>" class="w-full border border-green-300 px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
      </div>

      <div>
        <label for="first_name" class="block font-semibold text-green-800">👤 First Name</label>
        <input type="text" id="first_name" name="first_name" value="<?=html_escape($user['first_name']);?>" class="w-full border border-green-300 px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
      </div>

      <div>
        <label for="email" class="block font-semibold text-green-800">📧 Email</label>
        <input type="email" id="email" name="email" value="<?=html_escape($user['email']);?>" class="w-full border border-green-300 px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
      </div>
